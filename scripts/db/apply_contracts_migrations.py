...
MIGRATIONS = [
    ROOT / "database" / "flyway" / "sql" / "V006__contracts_view.sql",
    ROOT / "database" / "flyway" / "sql" / "V007__contracts_schema_bootstrap.sql",
    ROOT / "database" / "flyway" / "sql" / "V008__contracts_view_finalize.sql",
]
...
def main() -> int:
    ...
    try:
        for migration in MIGRATIONS:
            ...
            cur.execute(sql)
            conn.commit()
        print("OK: contracts migrations applied.")
        return 0
    finally:
        cur.close()
        conn.close()
