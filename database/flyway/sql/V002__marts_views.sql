-- 002_marts_views_oracle.sql
-- Oracle views (execute as user with access to MART + APP schemas)
...
select
  ...
  nvl(m.nome, '-') as morador,
  to_char(f.competencia, 'MON/YYYY', 'NLS_DATE_LANGUAGE=PORTUGUESE') as referencia,
  f.vencimento,
