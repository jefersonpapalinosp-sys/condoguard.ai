...
import {
  createSabespRun,
  fetchSabespRunDetail,
  fetchSabespRuns,
  type SabespConsumptionInput,
  type SabespRun,
  type SabespRunDetail,
  type SabespRunStatus,
} from '../../../services/sabespService';
import { DataSourceBadge } from '../../../shared/ui/DataSourceBadge';
import { LoadingState } from '../../../shared/ui/states/LoadingState';
import { ErrorState } from '../../../shared/ui/states/ErrorState';
import { EmptyState } from '../../../shared/ui/states/EmptyState';

const statusLabel: Record<SabespRunStatus, string> = {
...
const statusClass: Record<SabespRunStatus, string> = {
...
type ConsumptionRow = SabespConsumptionInput & { _key: number };

function emptyRow(key: number): ConsumptionRow {
  return {
    _key: key,
    unit: '',
    readingDate: '',
...
export default function SabespIntegrationPage() {
  const [rows, setRows] = useState<ConsumptionRow[]>([emptyRow(0)]);
  const [notes, setNotes] = useState('');
  const [submitting, setSubmitting] = useState(false);
  const [submitResult, setSubmitResult] = useState<SabespRunDetail | null>(null);
  const [submitError, setSubmitError] = useState<string | null>(null);

  const [runs, setRuns] = useState<SabespRun[]>([]);
  const [runsLoading, setRunsLoading] = useState(true);
  const [runsError, setRunsError] = useState<string | null>(null);
  const [runsMeta, setRunsMeta] = useState({ page: 1, totalPages: 1, hasNext: false, hasPrevious: false });
  const [statusFilter, setStatusFilter] = useState<SabespRunStatus | 'all'>('all');
  const [page, setPage] = useState(1);

  const [selectedRun, setSelectedRun] = useState<SabespRunDetail | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setRunsLoading(true);
        const response = await fetchSabespRuns({
          page,
          pageSize: 10,
  ...
  function updateRow(key: number, field: keyof SabespConsumptionInput, value: string) {
    setRows((current) =>
      current.map((r) => {
        if (r._key !== key) return r;
        if (field === 'amount' || field === 'consumptionM3') {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    const validRows = rows.filter((r) => r.unit.trim() && r.readingDate && r.dueDate && r.consumptionM3 > 0 && r.amount > 0);
    if (validRows.length === 0) {
      setSubmitError('Adicione ao menos um item com unidade, leitura, vencimento, consumo e valor.');
      return;
    }

    try {
      setSubmitting(true);
      const result = await createSabespRun({ notes: notes.trim() || undefined, items: validRows });
  ...
  async function openDetail(runId: string) {
    try {
      ...
      const detail = await fetchSabespRunDetail(runId);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 md:space-y-8 md:p-8">
      <section className="rounded-3xl bg-[linear-gradient(135deg,#00324d_0%,#00587a_60%,#007c91_100%)] p-5 text-white shadow-xl md:p-7">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="max-w-2xl">
            <p className="text-[11px] uppercase tracking-[0.2em] text-white/75">Integracao</p>
            <h2 className="mt-2 font-headline text-2xl font-extrabold tracking-tight md:text-4xl">Importacao Sabesp</h2>
            <p className="mt-2 text-sm text-white/85 md:text-base">
              Importe leituras e contas de agua da Sabesp em lote para apoiar a gestao de consumo hidrico.
            </p>
          </div>
          <DataSourceBadge module="sabesp" />
        </div>
      </section>

      <section className="rounded-2xl border border-outline-variant/30 bg-surface-container-low p-5">
        <h3 className="mb-4 font-headline text-lg font-bold">Nova importacao</h3>

        {submitResult && (
          <div className="mb-4 rounded-xl border border-tertiary-fixed-dim/40 bg-tertiary-fixed-dim/20 p-4">
            ...
            <table className="w-full min-w-[860px] text-xs">
              <thead>
                <tr className="text-left text-[10px] uppercase tracking-widest text-on-surface-variant">
                  ...
                  <th className="pb-2 pr-2">Leitura *</th>
                  <th className="pb-2 pr-2">Vencimento *</th>
                  <th className="pb-2 pr-2">Consumo m3 *</th>
                ...
                {rows.map((row) => (
                  <tr key={row._key}>
                    ...
                    <td className="py-1 pr-2">
                      <input
                        value={row.resident ?? ''}
                        onChange={(e) => updateRow(row._key, 'resident', e.target.value)}
                        placeholder="Nome"
                        className="w-full rounded-lg border border-outline-variant/30 bg-surface-container-highest px-2 py-1.5 outline-none focus:ring-1 focus:ring-primary-fixed"
                      />
                    </td>
                    <td className="py-1 pr-2">
                      <input
                        value={row.reference ?? ''}
                        onChange={(e) => updateRow(row._key, 'reference', e.target.value)}
                    ...
                    <td className="py-1 pr-2">
                      <input
                        ...
                        onChange={(e) => updateRow(row._key, 'readingDate', e.target.value)}
                        className="w-full rounded-lg border border-outline-variant/30 bg-surface-container-highest px-2 py-1.5 outline-none focus:ring-1 focus:ring-primary-fixed"
                      />
                    </td>
                    <td className="py-1 pr-2">
                    ...
                    <td className="py-1 pr-2">
                      <input
                        ...
                        step="0.01"
                        min="0.01"
                        value={row.consumptionM3 || ''}
                        onChange={(e) => updateRow(row._key, 'consumptionM3', e.target.value)}
                        placeholder="0,00"
                        className="w-full rounded-lg border border-outline-variant/30 bg-surface-container-highest px-2 py-1.5 outline-none focus:ring-1 focus:ring-primary-fixed"
                      />
                    </td>
                    ...
                    <td className="py-1">
                      {rows.length > 1 && (
                        <button
                          ...
                          className="rounded bg-error-container px-2 py-1.5 text-[10px] font-bold text-on-error-container hover:opacity-80"
                        >
                          X
                        </button>
                      )}
          ...
          <div className="flex flex-wrap gap-2">
            {(['all', 'completed', 'completed_with_errors', 'failed'] as const).map((s) => (
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-[10px] font-bold transition-colors ${statusFilter === s ? 'bg-primary text-on-primary' : 'bg-surface-container-highest'}`}
              >
                {s === 'all' ? 'Todos' : statusLabel[s as SabespRunStatus]}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40" onClick={() => setSelectedRun(null)}>
          ...
          >
            ...
            {selectedRun.errorSummary && (
              <div className="mb-4 rounded-xl bg-error-container/40 p-3 text-xs text-on-error-container">{selectedRun.errorSummary}</div>
            )}

            <h4 className="mb-3 text-sm font-bold">Itens ({selectedRun.items?.length ?? 0})</h4>
