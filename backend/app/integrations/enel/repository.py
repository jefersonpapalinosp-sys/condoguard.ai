...
# ---------------------------------------------------------------------------
# Oracle persistence helpers (Sprint 13 — S13-03)
# ---------------------------------------------------------------------------

async def _persist_run_oracle(condominium_id: int, run_record: dict[str, Any]) -> None:
    """Best-effort: persist completed ENEL run to app.integracoes_execucoes + app.integracoes_itens.
    Errors are swallowed — JSON state is the authoritative fallback."""
    try:
...
async def _get_run_detail_oracle(condominium_id: int, run_id: str) -> dict[str, Any] | None:
    """Fetch full run + items from Oracle. Returns None on error or not found."""
    try:
        ...
        item_rows = await run_oracle_query(
            ...
            select item_index, result, reason, external_reference, unit_code,
                   resident_name, reference_code, due_date, amount, invoice_status,
                   business_key, external_hash
        ...
        items = [
            {
                ...
                "raw": {
                    ...
                    "status": str(r.get("INVOICE_STATUS") or "pending"),
                },
            }
...
async def list_enel_runs(condominium_id: int, page: int = 1, page_size: int = 20, status: str | None = None) -> dict[str, Any]:
    ...
    if settings.db_dialect == "oracle":
        ...
        record_api_fallback_metric("integrations_enel", "oracle_list_runs_fallback")
    state = await read_json_state(ENEL_STATE_FILE)
    tenant = _safe_tenant_state(state, condominium_id)
...
async def get_enel_run_detail(condominium_id: int, run_id: str) -> dict[str, Any] | None:
    ...
    if settings.db_dialect == "oracle":
        ...
        if oracle_run is not None:
            return oracle_run
        record_api_fallback_metric("integrations_enel", "oracle_run_detail_fallback")
    state = await read_json_state(ENEL_STATE_FILE)
    tenant = _safe_tenant_state(state, condominium_id)
    runs = tenant.get("runs", [])
    for run in runs:
