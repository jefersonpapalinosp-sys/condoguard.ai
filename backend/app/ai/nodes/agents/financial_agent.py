...
async def financial_agent_node(state: AgentState) -> dict:
    from app.ai.chains import build_domain_chain  # noqa: PLC0415 — lazy for test patching
    ...
    try:
        ...
        return {
            "agent_response": rule_based_response(
                classification.get("intentId", "invoices_overview"), context, state.get("question", "")
            ),
